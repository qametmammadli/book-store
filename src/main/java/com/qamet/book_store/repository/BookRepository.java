    ...
    default Page<Book> findAllBySpec(BookSpecDTO bookSpecDTO, Pageable pageable) {
        return findAll((root, query, criteriaBuilder) -> {
                    ...
                    if (bookSpecDTO.getBookName().length() > 0) {
                        predicates.add(criteriaBuilder.like(root.get(Book_.NAME), "%" + bookSpecDTO.getBookName() + "%"));
                    }

                    if (bookSpecDTO.getBookDescription().length() > 0) {
                        predicates.add(criteriaBuilder.like(root.get(Book_.DESCRIPTION), "%" + bookSpecDTO.getBookDescription() + "%"));
                    }
                    ...
                    if (bookSpecDTO.getPublisherName().length() > 0) {
                        ...
                        Predicate publisherNamePredicate = criteriaBuilder.like(
                                publisherNameConcatExpression,
                                "%" + bookSpecDTO.getPublisherName() + "%");
                        predicates.add(publisherNamePredicate);
                    }

                    if (bookSpecDTO.getAuthorName().length() > 0) {
                        predicates.add(criteriaBuilder.like(root.join(Book_.AUTHORS).get(Author_.NAME), "%" + bookSpecDTO.getAuthorName() + "%"));
                    }
